  ...
  useEffect(() => {
    const handleAddCartItem = async () => {
      if (typeof id !== 'undefined' && productQty) {
        ...
        dispatch(CART_ADD_ITEM({ ...data, product: data._id }))
      }
    }
    handleAddCartItem()
  }, [id, productQty])
    ...
    <WebLayout>
      {Boolean(cartItems.length === 0) && (
        <Box
          ...
          p="10px"
        >
          <Text fontSize="2xl" color="#fff" mb="10px">
            Your cart is empty
          </Text>
          <Link
            to="/products"
            className="bg-white text-secondary-600 font-medium text-lg px-5 py-2 rounded-lg"
          >
            Go to market
          </Link>
        </Box>
